...
- 開発時は `WSL2` 上 build を主経路として成立させたい
- `flash` と `monitor` は `Windows` 側実行を許容し、実機 bring-up を止めないこと
...
- ローカルと CI の `ESP-IDF` version を一致させ、固定 version で再現可能にすること
...
- `開発専用 USB CDC` と `UART` のどちらを安定化後の標準 debug console にするか
